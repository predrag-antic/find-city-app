...
export class CityService {
    ...
    getCitiesByIndex(index){
        ...
        return fetch(`http://localhost:3000/cities?_sort=indices.${index}&_order=asc&_limit=15`)
        .then(response=>response.json())
        else{
            return fetch(`http://localhost:3000/cities?_sort=indices.${index}&_order=desc&_limit=15`)
        .then(response=>response.json())
        }
    }
